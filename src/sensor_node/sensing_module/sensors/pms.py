...
class PMS7003(Sensor):
    ...
    def calibrate(self):
        ...
        print(
            "Calibrating Sensor PMS7003 ({0} seconds)... done!".format(
    ...
    def _check_temperature_range(self, current_temperature=None):
        ...
        """
        if (
            current_temperature >= self._MIN_TEMPERATURE
            and current_temperature <= self._MAX_TEMPERATURE
        ):
            return True
        return False
    ...
    def _check_humidity_range(self, current_humidity=None):
        ...
        """
        if (
            current_humidity >= self._MIN_HUMIDITY
            and current_humidity < self._MAX_HUMIDITY
        ):
            return True
        return False
